...
const handler = async (m, { conn, isOwner, usedPrefix, command, text }) => {
  ...
  const lister = ['انشاء', 'رساله', 'حذف'];

  const [feature, inputs, inputs_, inputs__, inputs___] = text.split(' ');
  if (!lister.includes(feature)) {
    return conn.sendButton(m.chat, '*مثال:*\n' + usedPrefix + command + ' انشاء\n\n*اختر نوعًا موجودًا*\n' + lister.map((v, index) => '  ○ ' + v).join('\n'), null, [
      ['انـشـاء', `${usedPrefix}${command} انشاء`],
      ['الرسـالـه', `${usedPrefix}${command} رساله`], 
      ['حـذف', `${usedPrefix}${command} حذف`]
    ], [['قناتي ⚡', `https://whatsapp.com/channel/0029Vai5lsFCRs1tg6jJ152f`]], m);
  }
